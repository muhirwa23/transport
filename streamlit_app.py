...
def generate_live_data():
    ...
    new_data = {
        ...
        'latitude': -1.9441 + (0.001 * np.random.randn()),  # Use np.random
        'longitude': 30.0619 + (0.001 * np.random.randn()),  # Use np.random
        'type': np.random.choice(['Accident', 'Congestion']),
        'severity': np.random.randint(1, 5),
        'route': np.random.choice(['Route A', 'Route B', 'Route C']),
        'vehicle_count': np.random.randint(5, 50),
        'travel_time': np.random.randint(5, 30)
    }
    return new_data
